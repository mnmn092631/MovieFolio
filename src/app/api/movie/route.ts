...
export async function GET(req: NextRequest) {
	...
	if (!id) return NextResponse.json({ message: "Bad Request: Missing ID parameter" }, { status: 400 });
	...
	try {
		...
		const findMovieInfo: MovieInfoAPIData = await fetch(`${process.env.MOVIE_INFO_BY_ID_API}${id}`).then((data) =>
			data.json()
		);
		...
		const findMovie = await prisma.movie.create({
			data: {
				...
				watchGradeNm: movieInfo.audits.length > 0 ? movieInfo.audits[0].watchGradeNm! : "",
			},
		});
